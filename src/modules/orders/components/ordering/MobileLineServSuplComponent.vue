<template>
    ...
    <div class="sm:flex sm:items-center">
      <div class="sm:flex-auto">
        <h1 class="text-base font-semibold leading-6 text-primary-700 dark:text-primary-100">Asigne la SIMCARD y el Teléfono al servicio suplementario</h1>
              ...
              <tr v-for="item in data_question.data.answer.input_required" :key="item.id">
                <td class=" md:hidden whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 dark:text-primary-100 sm:pl-0 max-w-200 break-all">
                    ...
                    <div class="block w-full">
                        ...
                        <input value="Servicio Sumplementario" disabled type="text" />
                    </div>
                    <div class="block w-full">
                        ...
                        :disabled="item.action_sales_type==list_action_sales_type.find(x => x.name=='Servicio Suplementario').id"
                        type="text"
                ...
                <td class="hidden md:table-cell whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                    <input v-model="item.simcard" :disabled="item.action_sales_type==list_action_sales_type.find(x => x.name=='PRE a POS').id || item.action_sales_type==list_action_sales_type.find(x => x.name=='Servicio Suplementario').id"
                ...
                <td class="hidden md:table-cell whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                    ...
                    <template v-else-if="item.action_sales_type!='' && item.action_sales_type!=null && item.action_sales_type == list_action_sales_type.find(x => x.name=='Servicio Suplementario').id">
                        <select v-model="item.object_id_number" class="dark:text-white dark:bg-slate-700" @change="fn_get_simcard(item)">
                            <option v-for="item in g_systemRegisteredPhonesPOS" :key="item.celular" :value="item.celular">{{ item.celular }}</option>
                        </select>
                    </template>
...
const fn_validar = async (item) => {  
    ...
    try
    {
        ...
        if (item.action_sales_type === list_action_sales_type.value.find(x => x.name=='Servicio Suplementario').id) 
        {
            if (!item.object_id_number) {
                respValidaciones.value.push({ data: item, message: 'Ingrese un número de teléfono válido para Servicio Suplementario.' });
                item.status='ERROR';
                loading.value = false; return;
            }
        } 
...
onMounted(async () => {
    ...
    temp_list_action_sales_type.value.push(list_action_sales_type.value.find(item => item.name=='Servicio Suplementario'))
