    ...
    <div class="mb-2"> 
      <div class="mt-2 md:flex md:items-center md:justify-between">
        <div class="min-w-0 flex-1">
          <h2 class="text-2xl font-bold leading-7 text-gray-900 dark:text-primary-100 sm:truncate sm:text-3xl sm:tracking-tight">Buscar Ordenes</h2>
        </div>
        <div class="mt-4 flex flex-shrink-1 lg:flex-shrink-0 md:ml-4 md:mt-0">
            <InputText v-model="valuesearch" placeholder="ingrese cédula o pasaporte" class="lg:w-96" />
            <Button label="Buscar" icon="pi pi-search" size="small" @click="fn_search" />
         </div>
      </div>
    </div>   
    <div class="card"> 
        <DataTable v-model:filters="filters" :value="list_orders" paginator showGridlines :rows="10" dataKey="id"
            ...
            <template #header>
                <div class="flex justify-between ">
                    ...
                    <span class="relative ml-2 lg:ml-0">
...
const {a_searchOrder} = OrderStore();
const { list_orders, loading } =storeToRefs(OrderStore());
  ...
  const fn_search = async () => {
    await a_searchOrder(valuesearch.value);
  };
